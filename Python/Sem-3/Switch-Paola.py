...
frase = "y"  # permitira continuar
countador = 0  # auxiliar contador para el while


while True:

    aux_input = input("Quieres iniciar el proceso: y/n ").lower()
    ...
    ''' with switch(opcion) as solucion:
        if solucion.case("a", True):
            suma(numero1, numero2)
        if solucion.case("b", True):
            resta(numero1, numero2)
        if solucion.case("c", True):
            multiplicacion(numero1, numero2)
        if solucion.case("d", True):
            division(numero1, numero2)
        if solucion.default():
